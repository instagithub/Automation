...
public class BrowserImpl extends com.pega.BrowserImpl {
	...
	public <T extends Portal> T getPortal(Class<T> type) {
		...
		if (className.contains("WSSPortal")) {
			portal = type.cast(new WSSPortalImpl(testEnv));
		}
	...
	public void chatUserLogout() {
		...
		//List<WebElement> logout = pegaDriver.findElements(By.xpath(CS_IMPL_LOG_OFF_XPATH));
		//logout.get(1).click();
		//logout.get(0).click();
		pegaDriver.findElement(By.xpath(CS_IMPL_LOG_OFF_XPATH)).click(false);
		...
		try{
			TargetLocator switching = pegaDriver.switchTo();
			
			switching.alert().accept();
		}
		catch(Exception e){}
...
//		}
	...
	public void declineCall() {
		// pegaDriver.waitForDocStateReady(1);
		pegaDriver.switchTo().defaultContent();
		pegaDriver.findElement(By.xpath("//button[contains(.,'Decline')]")).click();
		// declineIcon.click();
	}

	public void changeVolumeAlert(String volume) {
		PegaWebElement callVolume = pegaDriver.findElement(By.xpath("//a[@title='Call volume alert ']"));
		callVolume.click();
		PegaWebElement callVolumeLow = pegaDriver.findElement(By.xpath("//input[@id='VolumeIndicatorLow']"));
		callVolumeLow.click();
		PegaWebElement closeButton = pegaDriver.findElement(By.id("container_close"));
		closeButton.click();

	}

	public void logoutasManager() {
		pegaDriver.waitForDocStateReady(2);

		pegaDriver.findElement(By.xpath(CS_SOCIAL_OPERATOR_XPATH)).click(false);
		pegaDriver.switchTo().defaultContent();

		pegaDriver.findElement(By.xpath(CS_SOCIAL_IMPL_LOGOFF_XPATH)).click(false);
	}

	public void logoffAutomationPortal() {
		pegaDriver.waitForDocStateReady(2);
		pegaDriver.switchTo().defaultContent();
		pegaDriver.findElement(By.xpath(DAN_LOGOFF_XPATH)).click(false);
		pegaDriver.switchTo().defaultContent();
