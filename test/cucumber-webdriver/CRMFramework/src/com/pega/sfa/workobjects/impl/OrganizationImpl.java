...
public class OrganizationImpl extends WizardImpl implements Organization {
	...
	public OrganizationImpl(WebElement elmt) {
		super(elmt);
	}
	
	public OrganizationImpl(WebElement elmt, String elmtId){
		super(elmt, elmtId);
	}
	...
	public Organization getOrganizationDetails( ) {
		...
		Organization orgDetails = new OrganizationImpl(framElmt, frameId);
		orgDetails._setEnvironment(testEnv, frameId);
		return orgDetails;	
	}
	
	public Organization editOrganization()
	
	{
		...
		pegaDriver.switchTo().frame(frameId);
		Organization editOrg = new OrganizationImpl(frameElmt, frameId);
		editOrg._setEnvironment(testEnv, frameId);
		return editOrg;
	}

	@Override
	public Organization changeOwner() {
		...
		Organization Org = new OrganizationImpl(frameElmt, frameId);
	...
	@Override
	public Organization changeHierarchy()
	{
		/*pegaDriver.findElement(By.xpath("//div[@string_type='field']//button[text()='Actions ']")).click();
		...
		*/
		UtilImpl.dropdown(pegaDriver, UtilImpl.ACTION_BUTTON_XPATH, "Change hierarchy");	
		String frameId = pegaDriver.getActiveFrameId(false);
		PegaWebElement frameElmt = pegaDriver.findElement(By.id(frameId));
		pegaDriver.switchTo().frame(frameId);
		frameId = pegaDriver.getActiveFrameId(false);
	...
	@Override
	public Organization setNewOwner(String owner) {
		...
		pegaDriver.waitForDocStateReady(2);*/
		UtilImpl.autoComplete(pegaDriver, "crmSearchOwner",owner);
	...
	@Override
	public Organization setHierarchy(String newParentOrg)
	{
		String frameId = pegaDriver.getActiveFrameId(false);
		PegaWebElement frameElmt = pegaDriver.findElement(By.id(frameId));
		pegaDriver.switchTo().frame(frameId);
		...
		UtilImpl.autoComplete(pegaDriver, "crmSearchTerm", newParentOrg);
		pegaDriver.waitForDocStateReady(2);		
		pegaDriver.findElement(By.xpath("//button[text()='Submit']")).click();
		return Org;	
	}
	...
	public void setTerritory(String territory) 
	{
		...
		pegaDriver.getActiveFrameId(true);*/
		UtilImpl.autoComplete(pegaDriver, ORG_TERRIORTY_ID, territory);	
	}
	...
	@Override
	public Organization navigateOrganization() {
		return null;
	}
	...
	public void setParentOrganization(String parentOrg) {
		...
		pegaDriver.getActiveFrameId(true);
		UtilImpl.autoComplete(pegaDriver, ParentOrg, parentOrg);
	...
	public Tasks addTask() {
		...
		PegaWebElement framElmt = pegaDriver.findElement(By.id(frameId));
		pegaDriver.switchTo().frame(frameId);
		Tasks tasksDetails = new TasksImpl(framElmt, frameId);
	...
	public Accounts AddAccount() {
		...
		PegaWebElement framElmt = pegaDriver.findElement(By.id(frameId));
		pegaDriver.switchTo().frame(frameId);
		Accounts AccountNewHarnessDetails = new AccountsImpl(framElmt, frameId);
	...
	Wizard wizard = pegaDriver.findWizard(pegaDriver.getActiveFrameId(false));
		...
		return(UtilImpl.getRowValues(pegaDriver, ORG_OPPTY_ROW_IDENTIFIER_XPATH, opptyName));
	...
	public List<String> getLeadRowValues(String companyName) {
		...
		return (UtilImpl.getRowValues(pegaDriver, LEAD_ROWS_XPATH, companyName));
	}		  
}